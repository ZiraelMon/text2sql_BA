...
def build_prompt(question, schema, db_id, answer=None):
    """Build improved prompt with explicit SQL start/end tokens."""
    prefix = (
        f"### Text-to-SQL task\n"
        f"Database: {db_id}\n"
        f"Schema:\n{schema}\n\n"
...
training_args = HFTrainingArguments(
    ...
    save_total_limit=2,
    load_best_model_at_end=False,  # Set to False to avoid issues
...
# Create generation config with proper tokens
...
# Set the generation config directly on the model instead
