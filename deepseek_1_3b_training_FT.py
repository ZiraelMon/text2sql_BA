...
# ════════════════════════════════════════════════════════════════
# Text-to-SQL Training – DeepSeek Coder 1.3B (Full fine-tuning)
# ════════════════════════════════════════════════════════════════
...
# ----- SAFE COLLATOR -----
def collate(batch):
    """Custom collator to handle tensors properly"""
    keys = batch[0].keys()
    out = {}
    for k in keys:
        if isinstance(batch[0][k], torch.Tensor):
            out[k] = torch.stack([f[k] for f in batch])
        else:
            out[k] = torch.tensor([f[k] for f in batch])
    return out
...
try:
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"Model size: Full checkpoint (~2.6 GB for 1.3B model)\n")
