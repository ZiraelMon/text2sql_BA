# -*- coding: utf-8 -*-
"""DeepSeek_1.3b_Evaluation.ipynb
...
# Spider-Evaluation für DeepSeek 1.3B
#@title { vertical-output: true }
...
# Spider-Repository klonen (falls noch nicht vorhanden)
if not os.path.exists("/content/spider"):
    !git clone https://github.com/taoyds/spider

# Spider-Tabellen und Datenbanken vorbereiten
SPIDER_TABLES = f"{DRIVE_BASE_DIR}/datasets/spider/tables.json"
SPIDER_DB_DIR = f"{DRIVE_BASE_DIR}/datasets/spider/database"

# Überprüfen der Dateien
print("Überprüfung der Eingabedateien:")
print(f"- Gold-SQL: {os.path.exists(GOLD_FILE)}")
print(f"- Pred-SQL: {os.path.exists(PRED_FILE)}")
print(f"- Spider-Tabellen: {os.path.exists(SPIDER_TABLES)}")
print(f"- Spider-Datenbanken: {os.path.exists(SPIDER_DB_DIR)}")

# Einen kurzen Blick auf die Dateien werfen
print("\nInhalt der Gold-SQL (erste 3 Zeilen):")
if os.path.exists(GOLD_FILE):
    !head -n 3 "{GOLD_FILE}"

print("\nInhalt der Pred-SQL (erste 3 Zeilen):")
if os.path.exists(PRED_FILE):
    !head -n 3 "{PRED_FILE}"

# Tabellendatei kopieren, falls sie nicht im Spider-Verzeichnis ist
if not os.path.exists("/content/spider/tables.json") and os.path.exists(SPIDER_TABLES):
    !cp "{SPIDER_TABLES}" "/content/spider/tables.json"
    print("Spider-Tabellen kopiert.")

# Führe die Evaluierung durch
os.chdir("/content/spider")
print("\nStarte Spider-Evaluierung...")

# Nur Exact Match (EM) Evaluierung - schnell
!python evaluation.py --gold "{GOLD_FILE}" --pred "{PRED_FILE}" --db "{SPIDER_DB_DIR}" --table tables.json --etype match

print("\n=== Vollständige Evaluierung (Execution Accuracy) ===")
# Vollständige Evaluierung mit Execution Accuracy - dauert länger
!python evaluation.py --gold "{GOLD_FILE}" --pred "{PRED_FILE}" --db "{SPIDER_DB_DIR}" --table tables.json --etype all
