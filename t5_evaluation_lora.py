# -*- coding: utf-8 -*-
"""T5_Evaluation_LoRA.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Qn6JAyREAn8-Iev5atdQoUdhp_fxmqr2
"""
...
USE_BF16              = True
LOAD_IN_4BIT          = True   # Match this with training script setting
MERGE_ADAPTER         = False  # Set to True to merge adapters for faster inference
# ----------------------------------------------------------
...
with open(gold_file, "w", encoding="utf-8") as gold_f, \
    ...
    # `references`, `predictions`, and `eval_ds["db_id"]`
...
# Installiere NLTK und lade die erforderlichen Daten
!pip install -q nltk
import nltk
nltk.download('all')
...
# Definiere Pfade
DRIVE_BASE_DIR = "/content/drive/MyDrive/text2sql"
EXPERIMENT_NAME = "deepseek_coder_6.7b_lora_sql_schema_a100"
EVAL_DIR = f"{DRIVE_BASE_DIR}/eval_results/{EXPERIMENT_NAME}"
GOLD_FILE = f"{EVAL_DIR}/gold_sql.txt"
PRED_FILE = f"{EVAL_DIR}/pred_sql.txt"
DB_PATH = f"{DRIVE_BASE_DIR}/datasets/spider/database"
TABLES_JSON = f"{DRIVE_BASE_DIR}/datasets/spider/tables.json"
SPIDER_EVAL_DIR = f"{DRIVE_BASE_DIR}/datasets/spider/spider_eval"

# Überprüfe, ob alle nötigen Dateien existieren
required_files = {
    "Gold SQL": GOLD_FILE,
    "Pred SQL": PRED_FILE,
    "Tables JSON": TABLES_JSON,
    "Evaluierungsskript": f"{SPIDER_EVAL_DIR}/evaluation.py",
    "Process SQL": f"{SPIDER_EVAL_DIR}/process_sql.py"
}
...
if missing:
    print("FEHLER: Die folgenden Dateien fehlen:")
    for item in missing:
        print(f"  - {item}")
    raise FileNotFoundError("Erforderliche Dateien fehlen")
else:
    print("Alle erforderlichen Dateien gefunden.")

# Prüfe Datenbanken
if os.path.exists(DB_PATH):
    db_files = [f for f in os.listdir(DB_PATH) if f.endswith('.sqlite')]
    print(f"Gefunden: {len(db_files)} Datenbanken in {DB_PATH}")
else:
    print(f"WARNUNG: Datenbank-Verzeichnis {DB_PATH} nicht gefunden!")

# Zeige die ersten Zeilen der Dateien an
print("\nInhalt von gold_sql.txt (ersten 3 Zeilen):")
!head -n 3 "{GOLD_FILE}"

print("\nInhalt von pred_sql.txt (ersten 3 Zeilen):")
!head -n 3 "{PRED_FILE}"

# Führe die Evaluierung aus
print("\nStarte Evaluierung...")

# Wechsle zum Evaluierungsverzeichnis, damit process_sql.py korrekt importiert wird
# %cd "{SPIDER_EVAL_DIR}"

# Führe die Evaluierung mit allen erforderlichen Parametern aus
!python evaluation.py --gold "{GOLD_FILE}" --pred "{PRED_FILE}" --db "{DB_PATH}" --table "{TABLES_JSON}" --etype all

print(f"\nEvaluierung abgeschlossen für: {EXPERIMENT_NAME}")
