# T5 Text-to-SQL Fine-Tuning Script with LoRA
# For bachelor thesis on schema-enhanced Text-to-SQL generation
# VERSION: With LoRA for efficient fine-tuning
...
# Install LoRA and quantization dependencies
!pip install -q peft>=0.4 bitsandbytes>=0.42
...
TARGET_MODULES = ["q", "v", "k", "o"]  # T5 attention modules to apply LoRA to
LOAD_IN_4BIT = True
...
print(f"\nLoading model: {MODEL_NAME} with 4-bit quantization and LoRA adapters...")
...
print("Model and tokenizer loaded with LoRA configuration.")
...
try:
    ...
    # Save only the LoRA adapters for efficiency
...
try:
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write("Adapter size: only ~1.6% of full model parameters\n")
...
# Print a note about adapter usage
print("\nIMPORTANT NOTES:")
print("1. Only the LoRA adapter weights have been saved (~30-50MB)")
print("2. To use this model for inference, you need to:")
print("   a. Load the base model: google/flan-t5-large")
print("   b. Load the adapter weights from:", training_args.output_dir)
print("   c. Merge them or use them with PeftModel.from_pretrained()")
print("3. Example inference code:")
print("```python")
print("from transformers import AutoModelForSeq2SeqLM, AutoTokenizer")
print("from peft import PeftModel")
print("base = AutoModelForSeq2SeqLM.from_pretrained('google/flan-t5-large')")
print(f"model = PeftModel.from_pretrained(base, '{training_args.output_dir}')")
print("# Optional: model = model.merge_and_unload() # for faster inference")
print("```")
